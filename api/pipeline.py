...
def transform_amount_encode(value, field):
    ...
    if len(padded) > field["encoded_length"]:
        return None, f"amount: {value} exceeds 8-digit capacity"
    if is_negative:
        last = padded[-1]
...
def transform_uppercase_passthrough(value, field):
    ...
    if len(v) != field["encoded_length"]:
        return None, f"{field['source_col']}: must be exactly {field['encoded_length']} characters (got {len(v)})"
    if not re.match(r'^[A-Z0-9]+$', v):
        return None, f"{field['source_col']}: '{v}' contains invalid characters"
    return v, None
...
def agent_transform_and_encode(data):
    ...
    for i, row in enumerate(data, 2):
        ...
        if record_errors:
            ...
            continue
        line = "".join(record.get(f["source_col"], "") for f in FIELDS)
...
def run_pipeline(file_bytes, filename):
    result = {
        ...
        "total_input_rows": 0, "successful_records": 0,
        "encoded_output": "", "checksum": "", "summary": "",
    }
    ...
    if any(e.get("severity") == "blocking" and "row" not in e for e in dq_errors):
...
class handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        body = json.dumps({"status": "Insurance ETL API is running"}).encode()
        self.send_response(200)
